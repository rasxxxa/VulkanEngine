...
void RunWindow()
{
    ...
    glfwWindowHint(GLFW_CLIENT_API, GLFW_NO_API);
    GLFWwindow* window2 = glfwCreateWindow(1280, 720, "Dear ImGui GLFW+Vulkan example", NULL, NULL);
    ...
    VkResult err = glfwCreateWindowSurface(g_Instance, window2, g_Allocator, &surface);
    check_vk_result(err);

    // Create Framebuffers
    int w, h;
    glfwGetFramebufferSize(window2, &w, &h);
    ...
    ImGui_ImplGlfw_InitForVulkan(window2, true);
